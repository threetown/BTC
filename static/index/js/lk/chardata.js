...
$('.amount-box').click(function(){
	...
	var bei=1;
	if($('#chbjjprice').get(0).checked){
		bei=$('#chbjjprice').val();
	}
    var yuqi = (( (order_shouyi*0.01*order_price)*10*bei+order_price*10) /10).toFixed(2);
    $('#yuqi').html(yuqi)
});
$('#chbjjprice').click(function(){
	var bei=1;
	if($('#chbjjprice').get(0).checked){
		bei=$('#chbjjprice').val();
	}
...
function addorder(){
	...
	if($('#chbjjprice').get(0).checked){
		postdata=postdata+'&ptype=jjprice';
		if(order_price > my_jjprice){
			err_info('奖金不足');
			return;
		}
	}
...
function endtimes() {
    ...
    $('.pay_order_sen').html(hours+":" + minutes+":"+seconds+"");
