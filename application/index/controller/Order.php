...
class Order extends Base
{
	...
	public function addorder()
	{
		...
		*/
		...
		//产品
		$pro=Db::name('productinfo')->where('pid',$data['order_pid'])->find();
		if(!$pro){
			
			return WPreturn('产品不存在',-1);
		}
		$time=time();
		if($pro['procode']=='btc'){
			...
			$protime=$pro['protime'];
			if($time-$dtime>$protime*60){
				$dtime=$dtime+24*60*60+$protime*60;
			}else{
				
				$dtime=$dtime+$protime*60;
			}
			$data['order_sen']=$dtime-$time;
		...
		$ptype=0;
		$ptypestr="usermoney";
		if(isset($data['ptype'])&&$data['ptype']=='jjprice'){
        ...
        //会员建仓后金额
        $adddata['commission'] = $user[$ptypestr] - $allfee;
        //订单号
        ...
        if($ids){
        	...
        	set_price_log($data['uid'],2,$u_fee,'下单','下单成功',$ids,$nowmoney,$ptype);
        	
        	if($editmoney){
				//发代数奖
				if($ptype==0){
					$tzdaishu=$conf['tzdaishu'];
					$wuxiandaishu=$conf['wuxiandaishu'];
					$dsarr=explode(',',$tzdaishu);
					$tj=Db::name('userinfo')->where('uid',$user['oid'])->find();
					$i=0;
					while($tj){
						$price=0;
						$title='';
						$cont='';
						if($i<count($dsarr)){
							$price=$data['order_price']*$dsarr[$i]*0.01;
							$title='代数奖';
							$cont='来自第['.($i+1).']代['.$user['username'].']';
						}else{
							if($tj['rank']==1){
								
								$price=$data['order_price']*$wuxiandaishu*0.01;
								$title='无限代奖';
								$cont='来自第['.($i+1).']代['.$user['username'].']';
							}
						...
						if($price>0){
							$editmoney1 = Db::name('userinfo')->where('uid',$tj['uid'])->setInc('jjprice',$price);
							if($editmoney1){
								set_price_log($tj['uid'],0,$price,$title,$cont,$ids,$tj['jjprice'],1);
							}
						}
						
						
						$i++;
						$tj=Db::name('userinfo')->where('uid',$tj['oid'])->find();
				...
				//发代数奖end
				//更新用户等级
				if($user['rank']==0&&$ptype==0){
					$allfee = Db::name('order')->where(array('uid'=>$data['uid']))->sum('fee');
					$allfee = $allfee?$allfee:0;
					$dailitouzi=$conf['dailitouzi'];
					if($allfee>=$dailitouzi){
						$editmoney1 = Db::name('userinfo')->where('uid',$data['uid'])->setInc('rank',1);
