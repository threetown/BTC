...
class Api extends Controller{
		...
		foreach ($orderlist as $k => $v) {
			...
			if($v['ostyle'] == 0){
				if($order_cha > 0){  //盈利
					...
					//平仓增加用户金额
                   	$db_userinfo->where('uid',$v['uid'])->setInc('jjprice',$yingli);
			...
			//买跌
				...
				if($order_cha < 0){  //盈利
					$yingli = $v['fee']*$winprice*0.01;
					$d_map['is_win'] = 1;
					

					//平仓增加用户金额
                   	$u_add = $yingli;
                   	$db_userinfo->where('uid',$v['uid'])->setInc('jjprice',$u_add);
			...
			if($yingli>0&&$ptype==0){
				...
				if($user){
					...
					$oid=$user['oid'];
					for($i=0;$i<count($dsarr);$i++){
						
						$tj=$db_userinfo->where('uid',$oid)->find();
						if($tj){
							$price=$v['fee']*$dsarr[$i]*0.01;
							$title='猜中代数奖';
							$cont='来自第['.($i+1).']代['.$user['username'].']';
							if($price>0){
								$editmoney1 = $db_userinfo->where('uid',$tj['uid'])->setInc('jjprice',$price);
								if($editmoney1){
									set_price_log($tj['uid'],0,$price,$title,$cont,$v['oid'],$tj['jjprice'],1);
								}
							}
							$oid=$tj['oid'];
	...
	public function set_order_log($v,$addprice)
	{
		$o_log['uid'] = $v['uid'];
       	...
       	//资金日志
       	set_price_log($v['uid'],1,$addprice,'结单','订单到期获利结算',$v['oid'],$o_log['user_money'],$v['ptype']);
	}
