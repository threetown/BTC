...
class User extends Base
{
	 ...
	 * @return [type] [description]
	 */
	public function jjtoprice()
	{
		$price = input('price');
		if($price&&is_numeric($price)&&$price>0){
			$user=$this->user;
			$data=array();
			$myjjprice=$user['jjprice'];
			if($myjjprice<$price){
				$data['status']=0;
				$data['msg']='奖金不足';
			...
			$editmoney1 = Db::name('userinfo')->where('uid',$user['uid'])->setInc('usermoney',$price);
			$editmoney = Db::name('userinfo')->where('uid',$user['uid'])->setDec('jjprice',$price);
				$data['status']=1;
				$data['msg']='操作成功';
				$res = json_encode($data);
				return $res;
	...
	public function huiqingzf(){
		...
		$merchant_id = '2386';  //商家Id
		$merchant_key = 'ed681704842045498ccb3c32fb58b426'; //商家密钥
