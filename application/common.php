...
function checkcode($code)
{
	...
	$codeid = Db::name('userinfo')->where(array('token'=>$code))->value('uid');
	if($codeid){
		return $codeid;
	}else{
		return false;
...
function set_price_log($uid,$type,$account,$title,$content,$oid=0,$nowmoney,$ptype=0)
{
		...
		$html .= '<div style="display:none" class="oid_list oid_'.$vo['oid'].'">
	                  <div class="vo_son" style="margin-left: '.$margin_left.'px;"><p>|——'.$type.'级</p></div>
	                    <div class="div_my_son">
	                      <ul class="my_sons">
	                        <li>名称：'.$vo['username'].' 余额：'.$vo['usermoney'].'</li>
